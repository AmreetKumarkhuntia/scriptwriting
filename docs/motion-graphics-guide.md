...
## Full-timeline faceless coverage (gap clips)

Amreet Talks long-forms are **fully faceless**: the creator's recording is VO only, and
full-screen graphics cover 100% of the runtime — dense *takeover* clips for the hooks plus
calmer *gap* clips for the explainer body, dropped back-to-back on V2 (every clip's end
frame = the next clip's drop frame). Face-overlay/PiP formats were tried once and rejected.

- **Grammar**: gap clips run one camera station per 6–9s (station–hold–drift; a new element
  enters at least every ~5s during holds), takeovers one per 3–4s. Shared scaffold:
  `remotion-all/src/AIKillSwitch/clips/gapKit.tsx` (`GapClip`, `gapCam` — emits pop-free
  drift keys because `camAt()` lerps from the previous key's *target*), worked examples in
  `clips/Gap1..9.tsx`.
- **Delivery**: one FCPXML assembly (all clips on the spine + every SFX cue on audio lanes,
  generated by `scripts/killswitch_sfx_fcpxml.py`) — the editor imports one file, adds VO +
  music, mutes baked audio.
- Full pipeline, beat-sync method, and the gotcha list live in the
  **`faceless-explainer-build`** skill (`.claude/skills/faceless-explainer-build/SKILL.md`).
