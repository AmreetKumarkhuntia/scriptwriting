...
import { Panel, PanelSub, PanelTitle, RosetteSeal } from "../mg/Cards";
...
// The "race" bracket: three lab cards + VS marks. Then the giant flat
// government seal DROPS over the whole bracket (comparison stays framed —
// camera only pulls back to make room), and the real player is named.
// ---------------------------------------------------------------------------
...
const CAM: MoKey[] = [
  { f: 0, cam: { cx: 1600, cy: 880, z: 0.94 } },
  { f: 10, cam: { cx: 1600, cy: 880, z: 1.0 }, trans: 60, mode: "glide" },
  { f: 140, cam: { cx: 1600, cy: 900, z: 0.86 }, trans: 44, mode: "settle" },
];
...
export const SealDrop: React.FC = () => {
  ...
  const sealY = interpolate(drop, [0, 1], [-1150, 210]);
    ...
    <AbsoluteFill style={{ backgroundColor: noir.void }}>
      ...
      <EnterExit>
        ...
        <MoStage keys={CAM}>
          ...
          {/* THE SEAL — drops over everything */}
          {frame >= DROP - 4 ? (
            <div
              style={{
                position: "absolute",
            ...
            >
              <RosetteSeal size={940} spin={interpolate(drop, [0, 1], [-14, 3])} />
            </div>
          ) : null}
        </MoStage>
